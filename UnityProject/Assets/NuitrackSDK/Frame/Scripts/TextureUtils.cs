...
{
    //Utilities for working with textures (caching, joining, copying, etc.)
    public class TextureUtils : MonoBehaviour
    {
        ...
        public void Copy(Texture2D source, ref RenderTexture dest)
        {
            ...
            var textureFormat = ConvertEnum<RenderTextureFormat>(source.format);
        ...
        public void Copy(RenderTexture source, ref Texture2D dest, TextureFormat texFormat = TextureFormat.ARGB32)
        {
            if (EqualsEnum<TextureFormat>(source.format))
                texFormat = ConvertEnum<TextureFormat>(source.format);

            if (dest == null || dest.width != source.width || dest.height != source.height || dest.format != texFormat)
                dest = new Texture2D(source.width, source.height, texFormat, false, true);
