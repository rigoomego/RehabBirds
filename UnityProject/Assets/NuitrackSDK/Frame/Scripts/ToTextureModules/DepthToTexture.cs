...
namespace NuitrackSDK.Frame
{
    public class DepthToTexture : FrameToTexture<DepthFrame, ushort>
    {
        ...
        public float MaxSensorDepth => maxDepthSensor;
        ...
        public int Width => NuitrackManager.sensorsData[0].DepthSensor.GetOutputMode().XRes;

        public int Height => NuitrackManager.sensorsData[0].DepthSensor.GetOutputMode().YRes;
        ...
        RenderTexture GetGPUTexture(DepthFrame frame, TextureCache textureCache, Gradient gradient)
        {
            ...
            else
            {
                ...
                if (destTexture == null)
                    destTexture = InitRenderTexture(frame.Cols, frame.Rows, RenderTextureFormat.ARGB64);
        ...
        public Texture2D GetTexture2D(DepthFrame frame, Gradient gradient, TextureCache textureCache = null)
        {
            ...
            if (GPUSupported)
            {
                ...
                FrameUtils.TextureUtils.Copy(cache.renderTexture, ref cache.texture2D, TextureFormat.RGBA64);
