...
public class NuitrackManager : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        {
            if (System.Environment.GetCommandLineArgs()[1] == "sensorId")
                sensorId = int.Parse(System.Environment.GetCommandLineArgs()[2]);
        }
#endif
        ...
        try
        {
            ...
            else
            {
                ...
                if (multisensorType == MultisensorType.Singlesensor)
                {
                    ...
                    {
                        devices.Add(currentDevices[sensorId]);
                        ...
                        if (customColorResolution)
                        {
                            ...
                            ChangeResolution(devices[0], colorWidth, colorHeight, deviceName, StreamType.COLOR);
                        }

                        if (customDepthResolution)
                        {
                            availableDepthResolutions = devices[0].GetAvailableVideoModes(nuitrack.device.StreamType.DEPTH);
                            ChangeResolution(devices[0], depthWidth, depthHeight, deviceName, StreamType.DEPTH);
                        }
                    }
                }
                else
    ...
    void ChangeResolution(NuitrackDevice sensor, int width, int height, string device, StreamType streamType)
    {
        VideoMode vm = new VideoMode();
        vm.width = width;
        vm.height = height;
        vm.fps = 30;
        sensor.SetVideoMode(streamType, vm);
    }
